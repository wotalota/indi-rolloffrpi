...
The purpose of the Raspberry Pi version of the roof driver is to eliminate the need for any coding when using an external controller. There will be a user interface panel for making the pin assignments and avoid the need to customise any code. This note outlines the prerequisites and limitations along with what was learned using gpio pins during testing.
